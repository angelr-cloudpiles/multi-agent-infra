...
export function validateRun(body, policy) {
  const allowed = new Set(['prompt','agent_id','alias','mode']);
  ...
  if (body.mode && !['task','meeting'].includes(body.mode)) throw new Error('Invalid mode');
  return {prompt:body.prompt.trim(),agent_id:body.agent_id,alias,mode:body.mode || 'task'};
}
