...
function PixelAgentsOffice({ statuses, selected, onSelect, onConversation, onTask, onReviewAttention, courier, readOnly = false }) {
    ...
    <div className="pixel-agents-office" role="group" aria-label="Oficina pixelada interactiva">
      ...
      {AGENTS.map((agent, index) => {
        ...
        return <div key={agent.id} className={`pixel-agent-control ${agent.color} ${active ? "active" : "idle"} ${dispatching ? "dispatching" : ""} ${attention ? "needs-attention" : ""} ${selected === agent.id ? "selected" : ""}`} style={{ "--station-x": `${x}%`, "--station-y": `${y}%`, "--portrait": `url(/pixel-agents/characters/char_${index}.png)` }}>
          ...
          {selected === agent.id && !readOnly && <div className="pixel-agent-actions" aria-label={`Acciones para ${agent.name}`}><button type="button" onClick={() => onConversation(agent.id)}>Conversar</button>{attention ? <button type="button" onClick={() => onReviewAttention(agent.id)}>Revisar atención</button> : <button type="button" onClick={() => onTask(agent.id)}>Asignar tarea</button>}</div>}
        </div>;
      })}
...
function OfficeScene({ snapshot, selected, onSelect, onConversation, onTask, onReviewAttention, connected, project, readOnly = false }) {
  ...
  return <section className="office-panel">
    ...
    <div className="office-scene"><PixelAgentsOffice statuses={statuses} selected={selected} onSelect={onSelect} onConversation={onConversation} onTask={onTask} onReviewAttention={onReviewAttention} courier={courier} readOnly={readOnly}/><div className="scene-key"><span><i className="key-working"/> Trabajando</span><span><i className="key-waiting"/> Espera</span><span><i className="key-idle"/> Disponible</span></div></div>
  </section>;
}
...
function AgentInspector({ selectedAgent, selectedState, selectedRun, selectedEvent, events, activity, project, onConversation, onReviewAttention, readOnly = false }) {
...
function ChatPanel({ messages, runs, agents, me, busy, onSend, onUpload, onFeedback, onRetry, onAssist, project, inputRef, preferredAgent, assistanceFor, onCancelAssistance, conversationTasks, onNewTask, compact = false, readOnly = false }) {
...
function App() {
  ...
  const project = configuredProjects.find((item) => item.project_id === projectId); const visualizerOnly = config.interface_mode === "visualizer"; const availableAgents = AGENTS.filter((item) => !project || (Array.isArray(project.allowed_agents) && project.allowed_agents.includes(item.id))); const runs = snapshot.runs || [];
  ...
  async function selectTask(run) {
    if (!run) return;
    setSelectedTask(run);
    setSelected(run.agent_id);
    setAssistanceFor(null);
    setConversationTasks(current=>({...current,[`${projectId}:${run.agent_id}`]:run.run_id}));
    try {
      const taskChat=await request(`/api/tasks/${run.run_id}/chat?project_id=${encodeURIComponent(projectId)}`);
      setFocusedTaskMessages(taskChat.messages||[]);
  ...
  return <main className="app-shell resizable-shell" style={gridStyle}>
    ...
    <section className="workbench"><header className="topbar"><div><span className="eyebrow">OPERACIONES</span><strong>{view === "office" ? "La oficina" : "Tareas del proyecto"}</strong></div><div className="top-meta"><span className="environment">{project?.environment || "PRODUCCIÓN"}</span><button className="logout" onClick={async () => { const result = await request("/auth/logout", { method: "POST", body: "{}" }); location.assign(result.url); }}>Salir</button></div></header><div className="main-content">{view === "office" && <><OfficeScene snapshot={{...snapshot,agents:AGENTS.map(agent=>({agent_id:agent.id,state:officeStates[agent.id]}))}} selected={selected} onSelect={setSelected} onConversation={openConversation} onTask={startTaskFor} onReviewAttention={reviewAttention} connected={Boolean(me)} project={project} readOnly={visualizerOnly}/><section className="tasks-preview"><div className="section-head compact-head"><div><span className="eyebrow">LISTA DE TAREAS</span><h2>En seguimiento</h2></div><button className="text-action" onClick={() => setView("tasks")}>Ver tareas <ArrowUpRight size={15}/></button></div><TaskList runs={runs} selectedRun={activeTask} onSelect={selectTask} compact/></section><ActivityFeed snapshot={snapshot} me={me}/></>}{view === "tasks" && <section className="tasks-page"><div className="section-head"><div><span className="eyebrow">LISTA DE TAREAS</span><h1>Trabajo del proyecto</h1><p>Selecciona una tarea para seguir sus resultados.</p></div>{!visualizerOnly&&<button className="primary" onClick={() => openConversation()}><Play size={16}/>Nuevo pedido</button>}</div><TaskList runs={runs} selectedRun={activeTask} onSelect={selectTask}/></section>}</div>{message && <p className="toast"><Activity size={15}/>{message}</p>}</section>
    ...
    <aside className="detail-column"><AgentInspector selectedAgent={selectedAgent} selectedState={selectedState} selectedRun={selectedRun} selectedEvent={selectedEvent} events={snapshot.events} activity={selectedActivity} project={project} onConversation={()=>openConversation(selected)} onReviewAttention={()=>reviewAttention(selected)} readOnly={visualizerOnly}/><TaskDetail run={activeTask} project={project} onApprove={approve} onAssist={startAssistance} canApprove={me?.canApprove} canAssist={Boolean(me?.sub)} busy={busy} readOnly={visualizerOnly}/>{project?.access_mode === "read_only_context" && <section className="scope-card"><LockKeyhole size={17}/><div><strong>Contexto de solo lectura</strong><p>Este piloto no puede cambiar AWS, CI/CD ni el repositorio.</p></div></section>}</aside>
    ...
    <aside className="conversation-column"><ChatPanel messages={assistanceFor || selectedTask ? focusedTaskMessages : messages} runs={runs} agents={availableAgents} me={me} busy={busy} onSend={sendMessage} onUpload={uploadAttachment} onFeedback={submitFeedback} onRetry={retrySynthesis} onAssist={startAssistance} project={project} inputRef={chatInputRef} preferredAgent={selected} assistanceFor={assistanceFor} onCancelAssistance={() => { setAssistanceFor(null); setFocusedTaskMessages([]); }} conversationTasks={conversationTasks} onNewTask={startNewTask} compact readOnly={visualizerOnly}/></aside>
  </main>;
}
