...
function OfficeScene({ snapshot, selected, onSelect, connected, project }) {
  ...
  return <section className="office-panel">
    <div className="office-heading"><div><span className="eyebrow">{project?.display_name || "OPERACIONES"} · {project?.environment || "PRODUCCIÓN"}</span><h1>La oficina</h1><p>{project?.access_mode === "read_only_context" ? "Piloto con contexto de solo lectura." : "El trabajo se muestra cuando sucede."}</p></div><span className={`connection ${connected ? "online" : "locked"}`}><i/>{connected ? "En directo" : "Acceso requerido"}</span></div>
...
function App() {
  const [snapshot, setSnapshot] = useState({ agents: [], events: [], runs: [] }); const [config, setConfig] = useState({ projects: [], default_project_id: "multi-agent" }); const [projectId, setProjectId] = useState("multi-agent"); const [me, setMe] = useState(null); const [selected, setSelected] = useState("research-agent"); const [prompt, setPrompt] = useState(""); const [agent, setAgent] = useState("research-agent"); const [busy, setBusy] = useState(false); const [message, setMessage] = useState("");
  const project = config.projects.find((item) => item.project_id === projectId); const availableAgents = AGENTS.filter((item) => !project || project.allowed_agents.includes(item.id));
  const refresh = async () => { try { const [identity, latest] = await Promise.all([request("/api/me"), request(`/api/snapshot?project_id=${encodeURIComponent(projectId)}`)]); setMe(identity?.sub ? identity : null); setSnapshot({ agents: latest.agents || [], events: latest.events || [], runs: latest.runs || [] }); setMessage(""); } catch (error) { if (error.message === "AUTH_REQUIRED") setMe(null); else setMessage(error.message); } };
  ...
  useEffect(() => { if (availableAgents.length && !availableAgents.some((item) => item.id === selected)) setSelected(availableAgents[0].id); if (availableAgents.length && !availableAgents.some((item) => item.id === agent)) setAgent(availableAgents[0].id); }, [projectId, config]);
  ...
  return <main className="shell"><header className="topbar"><a className="brand" href="/"><span className="brand-mark"><Bot size={20}/></span><strong>Agent <em>Office</em></strong></a><div className="top-meta"><span>{project?.display_name || "Multi Agent"}</span><span className="environment">{project?.environment || "PRODUCCIÓN"}</span>{me ? <button className="logout" onClick={async () => { const result = await request("/auth/logout", { method: "POST", body: "{}" }); location.assign(result.url); }}>Salir</button> : <a className="login" href="/auth/login">Entrar con Microsoft <ArrowUpRight size={15}/></a>}</div></header>
    <section className="workspace"><div className="office-column"><OfficeScene snapshot={snapshot} selected={selected} onSelect={setSelected} connected={Boolean(me)} project={project}/><ActivityFeed snapshot={snapshot} me={me}/></div><aside className="side-column"><section className="inspector"><span className="eyebrow">AGENTE SELECCIONADO</span><div className="inspector-identity"><span className={`pixel-portrait ${selectedAgent.color}`}><i/><b/></span><div><h2>{selectedAgent.name}</h2><p>{selectedAgent.role}</p></div></div><div className="state-line"><span className={`state-pip ${selectedState}`}/>{STATE_LABEL[selectedState]}</div><dl><div><dt>Tarea actual</dt><dd>{selectedRun ? selectedRun.prompt.slice(0, 100) : "Sin tarea asignada"}</dd></div><div><dt>Último evento</dt><dd>{selectedEvent?.type || "—"}</dd></div><div><dt>Trazas</dt><dd>{selectedEvent?.trace_id ? <a href="https://langfuse.aiops.cloudpiles.net" target="_blank" rel="noreferrer">Abrir Langfuse <ArrowUpRight size={14}/></a> : "Sin trazas"}</dd></div></dl></section>
      <section className="task-panel"><div><span className="eyebrow">NUEVA TAREA</span><h2>Encarga trabajo</h2></div><form onSubmit={createRun}><label>Proyecto<select value={projectId} onChange={(event) => { setProjectId(event.target.value); setSnapshot({ agents: [], events: [], runs: [] }); }} disabled={!config.projects.length || busy}>{config.projects.map((item) => <option value={item.project_id} key={item.project_id}>{item.display_name}</option>)}</select></label><label>Instrucciones<textarea value={prompt} onChange={(event) => setPrompt(event.target.value)} maxLength="12000" placeholder="Describe el resultado que necesitas…" disabled={!me || busy}/></label><label>Asignar a<select value={agent} onChange={(event) => setAgent(event.target.value)} disabled={!me || busy}>{availableAgents.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></label>{project?.access_mode === "read_only_context" && <p className="message"><LockKeyhole size={15}/>Contexto de solo lectura: no se ejecutan cambios ni accesos AWS.</p>}<button className="run" type="submit" disabled={!me || busy}><Play size={15}/>{busy ? "Enviando…" : "Ejecutar tarea"}</button></form><div className="approval"><h3>Aprobaciones</h3>{snapshot.runs.filter((run) => run.status === "waiting_for_approval").map((run) => <div className="approval-row" key={run.run_id}><span>Plan de despliegue</span>{me?.canApprove ? <span><button onClick={() => approve(run.run_id, "approve")} disabled={busy}>Aprobar</button><button onClick={() => approve(run.run_id, "reject")} disabled={busy}>Rechazar</button></span> : <small>Requiere aiops-approvers</small>}</div>) || <p>Sin aprobaciones pendientes.</p>}</div>{message && <p className="message"><Activity size={15}/>{message}</p>}</section></aside></section>
  </main>;
}
