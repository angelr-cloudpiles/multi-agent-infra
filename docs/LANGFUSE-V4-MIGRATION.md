...
| exports | partial | La infraestructura de batch export está habilitada en Langfuse hacia el bucket S3 cifrado `multi-agent-langfuse-traces-20260913111629427300000006` bajo `exports/`; el rol de Langfuse dispone de `s3:GetObject`, `s3:PutObject`, `s3:ListBucket` y KMS. Aún no hay objetos porque no se solicitó un export. La integración programada por proyecto (Blob Storage) requiere una clave con alcance de organización: las claves de proyecto devuelven `403`. Siguiente acción: crear esa clave y definir proyecto, prefijo, frecuencia, campos y retención antes de habilitarla. |
