...
| exports | changed | Langfuse OSS 4.35.0 no ofrece claves de organización ni Blob Storage gestionado; esa capacidad requiere Enterprise Edition. Se incorpora un task Fargate diario que usa exclusivamente las claves de cada proyecto y guarda Parquet cifrado en `exports/project-api/{project}/date={YYYY-MM-DD}/`. Exporta observaciones y scores, nunca los campos `input` u `output`; cada ejecución deja un manifiesto con conteos y SHA-256. |
