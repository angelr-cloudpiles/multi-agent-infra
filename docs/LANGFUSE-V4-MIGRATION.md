...
| exports | ready | Langfuse OSS 4.35.0 no ofrece claves de organización ni Blob Storage gestionado; esa capacidad requiere Enterprise Edition. El task Fargate diario habilitado usa exclusivamente las claves de cada proyecto y guarda Parquet cifrado en `exports/project-api/{project}/date={YYYY-MM-DD}/`. Exporta observaciones y scores, nunca los campos `input` u `output`; cada ejecución deja un manifiesto con conteos y SHA-256. La validación del 14 de septiembre creó los tres objetos de Gaudi (22 observaciones, 3 scores) y Tattoo Studio (68 observaciones, 3 scores), con salida KMS y esquema Parquet sin columnas de I/O. |
